...
const BasketIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 128 128">
    ...
    <path fill="#ffd58a" d="M60.01 99.47c.03.59-.39 1.15-.96 1.37c-1.78.71-6.48 1.83-11.97 1.34c-5.5-.49-7.81-2.12-9.69-3.15c-.6-.33-1.12-1.03-1.17-1.62l-.26-4.83c-.08-.92.64-1.23 1.63-.86c2.18.81 4.1 1.85 8.95 2.27s9.17.09 11.28-.36c.96-.2 1.99.53 2.04 1.45z"/>
    ...
    <path fill="#ffd58a" d="M60.73 120.32c0 .59-.45 1.37-1.57 1.46c0 0-4.61.74-10.68.09c-5.54-.59-9.97-2.79-9.97-2.79c-.84-.48-1.24-1.12-1.27-1.71l-.19-4.72c-.04-.92.69-1.37 1.62-.96c2.06.9 5.17 1.99 9.8 2.48c5.38.57 8.42.4 9.83.38s2.2.31 2.24 1.39z"/>
    ...
    <path fill="#d68b52" d="M63.46 39.99C36.15 39.99 14 43.43 14 47.68c0 1.48 2.71 2.86 7.39 4.04c8.07-2.4 23.89-4.04 42.07-4.04c18.19 0 34 1.63 42.07 4.04c4.68-1.17 7.39-2.56 7.39-4.04c0-4.25-22.14-7.69-49.46-7.69"/>
    <path fill="#894f36" d="M63.46 72.7c-22.56 0-47.68-4.21-57.28-11.22c0 0 .48 2.62 3.34 4.27c6.25 3.61 17.61 6.75 33.63 8.21c.33 1.12 1.63 3.32 6.74 3.32c4.67 0 6.58-1.39 7.37-2.52c2.02.05 4.08.08 6.21.08c25.58 0 44.97-4.12 53.91-8.86c0 0 1.06-.62 1.73-1.51s1.25-2.24 1.25-2.24c-3.16 2.76-23.51 10.47-56.9 10.47"/>
...
function Nav() {
  ...
  const { data: cart, refetch } = useApi(getCart, []);

  const basketCount = (() => {
    if (!cart) return 0;
    const items = cart.items ?? cart.cart_items ?? cart.data?.items ?? [];
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("roots:cart-updated", handler);
    ...
    <>
      <nav className={`nav ${scrolled ? "nav-scrolled" : ""}`}>
          ...
          {["Collection", "Origins", "Artisans", "About"].map((label) => {
            const handleClick = (e) => {
              if (label === "Collection") {
                e.preventDefault();
                const el =
                  document.querySelector("section.collection") ||
        ...
        <div className="nav-right">
          ...
          />
          <BasketButton count={basketCount} onClick={() => setCartOpen(true)} />
